...
type RunState = {
  ...
  gen: number; // مولّد حلقة التنفيذ — كل إحياء يزيده؛ الحلقات القديمة تنسحب فوراً
};
...
// ★ آمن للسيرفرلس (Vercel): لو انجمّدت الخلفية بين الطلبات، الاستفسار (status) يوقظها
//   من جديد — الحلقة القديمة لو صحيت تدرك أنها قديمة (gen) وتنسحب فوراً بلا نبضات مزدوجة
async function runRounds(s: RunState) {
  const gen = ++s.gen; // نأخذ ملكية التنفيذ — أي حلقة أقدم تنسحب تلقائياً
  const ct = s.ct;
  const stats = s.stats;
  const send = (ev: any) => writeEvent(s, ev);
  const isCancelled = () => s.cancelled || s.gen !== gen;
...
export async function POST(request: NextRequest) {
  try {
    ...
    if (mode === 'status') {
      ...
      // ★ وضع الاستضافة السيرفرلس (Vercel): الخلفية تنجمد أو تُقتل بين الطلبات
      // الاستفسار نفسه يوقّظ التنفيذ من حيث توقف — طلبات قصيرة بلا أي قناة تتنقطع
      // محلياً الخلفية ما تسكت أبداً (السكون أقل من 12.5ث) — هذا المسار لا يعمل أصلاً
      if (!s.done && !s.cancelled && Date.now() - s.lastBeatAt > 15000) {
        s.lastBeatAt = Date.now();
        pushLog(s, '↻ استئناف تلقائي للتنفيذ من حيث توقف');
    ...
    if (mode === 'selftest') {
      ...
      const s: RunState = { ct: 'selftest', questIds: [], phase: 'running', logs: [], quests: new Map(), stats: { completed: 0, claimed: 0, failed: 0 }, round: 0, done: false, cancelled: false, result: '', error: null, startedAt: Date.now(), lastBeatAt: Date.now(), gen: 0 };
      runs.set(id, s);
      void (async () => {
        for (let i = 1; i <= 20; i++) {
    ...
    if (mode === 'start') {
      ...
      const s: RunState = { ct, questIds, phase: 'starting', logs: [], quests: new Map(), stats: { completed: 0, claimed: 0, failed: 0 }, round: 0, done: false, cancelled: false, result: '', error: null, startedAt: Date.now(), lastBeatAt: Date.now(), gen: 0 };
