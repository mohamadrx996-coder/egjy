...
// الكويستات لها مسار نود مستقل /api/quests (التنفيذ الخلفي يحتاج نود لا إيدج)
...
// ✅ أي انهيار بأي أداة يرجع JSON واضح — أبداً صفحة خطأ HTML تخرب الواجهة
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    return NextResponse.json({ success: false, error: 'صار خطأ بالسيرفر أثناء التنفيذ — جرب مرة ثانية' }, { status: 500 })
  }
}
...
export async function GET(request: NextRequest) {
  ...
  try {
    ...
    console.error(`[api/${action}]`, e)
    return NextResponse.json({ success: false, error: 'صار خطأ بالسيرفر — جرب مرة ثانية' }, { status: 500 })
  }
}
